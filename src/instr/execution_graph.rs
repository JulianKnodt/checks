...
pub fn execution_graph(events: &Vec<Vec<MemOp>>) -> AdjList<Op, Relation> {
  ...
  (0..writes.len()).for_each(|i| {
    ...
    let no_prev_writes = 0 == (0..i).filter_map(|j| {
      ...
      Some(node_j).filter(|_| prev.mem_op.loc() == curr.mem_op.loc())
    })
  ...
  let reads : Vec<_> = out.nodes.iter().enumerate()
    .filter_map(|(i, n)| n.map(|op| (i, op)))
    .filter_map(|v| if v.1.mem_op.is_read() { Some(v) } else { None })
    .collect();

  // Add ReadFrom edges
  reads.iter()
    .for_each(|&(read_index, read)| match read.mem_op.data() {
      None => out.push_edge(init_state, read_index, Relation::ReadFrom(Locality::Global)),
      Some(data) => {
        let matching_writes : Vec<_> = writes.iter()
          .filter(|(_, w)| w.same_access_address(&read) && w.mem_op.data().unwrap() == data)
          .collect();
        if matching_writes.is_empty() {
          panic!("Defining graph for program is impossible, \
          no matching write for read: {:?}", events);
        }
